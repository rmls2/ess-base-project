Feature: Promoções API
    ...
    Scenario: Tentar registrar uma nova promoção com discountValue negativo
        Given a solicitação para registrar uma nova promoção é inválida com o request: user "pedro", adm "True", hotel "noite estrelada", room_id "womrjatajDEa1X0as0W7lVVb", reservationValue "150", discountValue "-20"
        When uma requisição "POST" é enviada para "promotions/register" 
    ...
    Scenario: Atualizar uma promoção existente
        Given existe uma promoção com ID do quarto "123" e a requisição tem user "pedro", adm "True", currentDiscountValue "25.0", newDiscountValue  "20.0" 
        When uma requisição "PUT" é enviada para "promotions/update"
        Then o status da resposta deve ser "200"
        And o JSON da resposta deve indicar o sucesso com o documento user "pedro", adm "True", hotel "noite estrelada", room_id "123", reservationValue "150.0", discountValue "20.0" e id "65d26a97e24aa3a20694e3aa"

    Scenario: Tentar atualizar uma promoção inexistente
        Given não existe uma promoção com ID do quarto "456" e a requisição tem user "pedro", adm "True", currentDiscountValue "25.0", newDiscountValue  "20.0"
        When uma requisição "PUT" é enviada para "promotions/update"
        Then o status da resposta deve ser "404"
        And o JSON da resposta deve conter a mensagem de erro "Promotion not found"

    Scenario: Excluir uma promoção
        Given existe uma promoção com ID do quarto "123" e a requisição tem user "pedro", adm "True"
        When uma requisição "DELETE" é enviada para "promotions/delete"
        Then o status da resposta deve ser "200"
    ...
    Scenario: Tentar excluir uma promoção inexistente
        Given não existe uma promoção com ID do quarto "456" e a requisição tem user "pedro", adm "True"
        When uma requisição "DELETE" é enviada para "/delete"
        Then o status da resposta deve ser "404"
        And o JSON da resposta deve conter a mensagem de erro "Promotion not found"